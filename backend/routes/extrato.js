...
router.get('/:id/export', async (req, res) => {
  ...
  try {
    ...
    // Enriquecer com saldo (simulado - no Prisma podes calcular ou buscar saldo atual)
    const transacoesComSaldo = transacoes.map(t => ({
      ...t,
      tipo_operacao: t.tipo?.nome_operacao || 'Outro',
      saldo_apos: contaPrincipal?.saldo || 0 // Simplificado - idealmente calculas o saldo histórico
    }));
...
function exportarPDF(res, cliente, transacoes, conta, data_inicio, data_fim) {
  ...
  // ✅ LIMITAR A 20 TRANSAÇÕES PARA CABER NUMA PÁGINA
