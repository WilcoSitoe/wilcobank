...
export default function Admin() {
  ...
  const handleDelete = async (id, nome) => {
    if (!window.confirm(`⚠️ Tem certeza que deseja eliminar o cliente "${nome}"?\nEsta ação é irreversível.`)) return;
    try {
  ...
  const handleDeposit = async (e) => {
    ...
    if (!valor || valor <= 0) { alert('⚠️ Insira um valor válido'); return; }
    try {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'Segoe UI' }}>
      ...
      <div style={{ 
        ...
        <h1 style={{ fontFamily: 'Rockwell', color: '#28283C', margin: 0 }}>🏦 WilcoBank Admin</h1>
                    ...
                    <tr key={c.id_cliente} style={{ borderBottom: '1px solid #eee', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                      ...
                      <td style={{ padding: '12px', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                          <button onClick={() => setDepositModal({ id_cliente: c.id_cliente, nome: c.nome_cliente, numero_conta: c.numero_conta })} style={{ padding: '6px 12px', background: '#2196F3', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>
                            💰
                          </button>
                          <button onClick={() => handleDelete(c.id_cliente, `${c.nome_cliente} ${c.apelido_cliente}`)} style={{ padding: '6px 12px', background: '#f44336', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>
                            🗑️
                          </button>
                        </div>
                      </td>
      ...
      {activeTab === 'relatorios' && (
        <div>
          {loadingRelatorios ? (
            ...
            <div style={{ background: '#fee', padding: '20px', borderRadius: '12px', color: '#c00' }}>
              ❌ Erro ao carregar relatórios. Verifica se a rota `/api/admin/dashboard` está configurada no backend.
            </div>
          ) : (
            <>
              {/* KPIs - COM NOVO CARD DE LUCRO COM TAXAS */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '25px' }}>
                <KpiCard titulo="Total Clientes" valor={relatorios.totalClientes} cor="#1e3c72" icone="👥" />
                <KpiCard titulo="Saldo Total Banco" valor={formatarMoeda(relatorios.saldoTotal)} cor="#2a5298" icone="🏦" />
                <KpiCard titulo="Depósitos (Mês)" valor={formatarMoeda(relatorios.depositosMes)} cor="#28a745" icone="📈" />
                <KpiCard titulo="Levantamentos (Mês)" valor={formatarMoeda(relatorios.levantamentosMes)} cor="#dc3545" icone="📉" />
              ...
              <div style={{ 
                ...
                <div>
                  <h4 style={{ margin: '0 0 5px 0', color: '#6a1b9a', fontFamily: 'Rockwell' }}>🧪 Testar Juros Automáticos</h4>
                ...
                >
                  {jurosTestLoading ? '⏳ A processar...' : '💰 Testar Juros Agora'}
                </button>
              </div>
              ...
              <div style={{ 
                ...
                <div style={{ fontSize: '13px', color: '#666' }}>
                  {searchTerm ? (
                    <span>📊 <strong>{filteredTransacoes.length}</strong> resultado(s) encontrado(s) para "<strong>{searchTerm}</strong>"</span>
                  ) : (
                    <span>📊 A mostrar <strong>{filteredTransacoes.length}</strong> transação(ões)</span>
                  )}
                </div>
              </div>
      ...
      {depositModal && (
        <div style={{
          ...
          <div style={{ background: 'white', padding: '30px', borderRadius: '12px', width: '90%', maxWidth: '400px', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}>
            <h3 style={{ margin: '0 0 15px 0', fontFamily: 'Rockwell', color: '#28283C' }}>
              💰 Depositar em {depositModal.nome}
            </h3>
